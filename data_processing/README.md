...
Small pipeline that pulls **daily archived forecasting weather** from Open-Meteo, merges it with **EPA county-level daily AQI** for **Tompkins / New York**, cleans the result, and writes a **modeling table** for downstream analysis or ML.
...
1. **Weather** — overwrites `data/raw/daily_weather_forecasting.csv` by default when you use `--tompkins` + daily mode (each row includes column `forecasting` = `open_meteo_historical_forecasting`):
...
| Script | Role |
|--------|------|
| `fetch_weather_forecasting_data.py` | Calls Open-Meteo **Historical Forecast** API; writes `forecasting` tag + `--freq hourly\|daily`, `--tompkins`, date range, `--out`. |
...
Units and meanings below match **this repo’s** `fetch_weather_forecasting_data.py` defaults (°C, mm, m/s for wind). See [Historical Forecast API docs](https://open-meteo.com/en/docs/historical-forecast-api) if you change units in the fetch script.
...
| `forecasting` | string | Series id from weather CSV (`open_meteo_historical_forecasting`). |
| `grid_latitude` | float | Nearest forecast-model grid latitude (°N). |
| `grid_longitude` | float | Nearest forecast-model grid longitude (°E, negative = W). |
...
Daily Open-Meteo historical-forecast export (same weather columns as above) **plus**:

| Column | Type | Description |
|--------|------|-------------|
| `forecasting` | string | Always `open_meteo_historical_forecasting` for this pipeline. |
...
- **Weather:** follow [Open-Meteo](https://open-meteo.com/) terms and attribution for the underlying **forecast models** (see [Historical Forecast API](https://open-meteo.com/en/docs/historical-forecast-api) and their License / data-source tables).
